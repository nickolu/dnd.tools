...
date: "2026-02-15"
description: "A narrative walk-through from compendium plumbing to shareable filters, admin ingestion, and the latest admin + monster upgrades."
---
...
With the ingestion mechanics in place, I turned it into a real working surface. I carved out dedicated admin routes and API handlers, added admin-specific button and textarea styles so the tools feel deliberate, and documented the OpenAI key in the `.env` example so the draft pipeline is easy to wire up on a new machine. On the monster side I made proficiency bonus a first-class schema field, surfaced it on the card, and tightened the tests so the display stays correct as data evolves.
