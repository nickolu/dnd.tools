...
import type { SpellListSelectorProps } from "@/page/spells/components/spell-list-selector/types";

export function SpellListSelector({
  activeListId,
  onListSelect,
}: SpellListSelectorProps) {
  const lists = useSavedSpellListStore((s) => s.lists);
  const createList = useSavedSpellListStore((s) => s.createList);
  const deleteList = useSavedSpellListStore((s) => s.deleteList);
  const renameList = useSavedSpellListStore((s) => s.renameList);
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        type="button"
      >
        {activeList ? activeList.name : "All Spells"}
      </button>
